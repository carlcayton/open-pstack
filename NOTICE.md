...
- `plugins/pstack/skills/poteto-mode/scripts/` is vendored from upstream (`watch-pr`, `orch`, `bootstrap.ts`, `worktree-audit.sh`, `package.json`, `bun.lock`) with two edits: `worktree-audit.sh` reads `~/.claude/projects/` instead of Cursor's transcript directory, and the private workspace package is renamed `@pstack-claude/poteto-mode-tools`. Everything else is upstream's code under the same MIT license.
